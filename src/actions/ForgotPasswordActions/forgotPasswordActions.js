...
export function forgotPassword(email, channel, verifyCode) {
  return dispatch => {
    dispatch(
      setForgotPasswordLogging(
        forgotPasswordConstants.FORGOT_PASSWORD_PENDING,
        null
      )
    );
    sendForgotPasswordRequest(email, channel, verifyCode)
      .then(success => {
        dispatch(
          setForgotPasswordLogging(
            forgotPasswordConstants.FORGOT_PASSWORD_SUCCESS,
            null
          )
        );
        if (verifyCode) {
          ...
          navigate('/forgot-password', { state: { step: 2 } });
        }
      })
      .catch(err => {
        dispatch(
          setForgotPasswordLogging(
            forgotPasswordConstants.FORGOT_PASSWORD_ERROR,
            err
          )
        );
      });
...
export function changePassword(accessToken, newPassword, conPassword) {
  return dispatch => {
    dispatch(
      setChangePasswordLogging(
        forgotPasswordConstants.CHANGE_PASSWORD_PENDING,
        null
      )
    );
    sendChangePasswordRequest(accessToken, newPassword, conPassword)
      .then(success => {
        dispatch(
          setChangePasswordLogging(
            forgotPasswordConstants.CHANGE_PASSWORD_SUCCESS,
            null
          )
        );
        navigate('/login');
      ...
      .catch(err => {
        dispatch(
          setChangePasswordLogging(
            forgotPasswordConstants.CHANGE_PASSWORD_ERROR,
            err
          )
        );
      });
  ...
  return new Promise((resolve, reject) => {
    axios
      .post(
        ...
        buildPayloadForgotPassword(email, channel, verifyCode)
      )
      .then(value => {
